...
#location = ["Example City", "0000000"]   # for the OpenWeather widget
wallpaper_background = '$HOME/Pictures/Wallpapers/wallpaper.png'
...
keys = [
    ...
    Key([mod, "shift"], "comma", lazy.function(WindowToPreviousScreen),
        desc="Move current window to previous screen"),
    Key([mod, "shift"], "period", lazy.function(WindowToNextScreen),
    ...
    # Power Menu
    Key([mod, "shift"], "x", lazy.spawn(f"bash {scripts}powermenu/type-1/powermenu.sh"),
        desc="Display power menu"),

    # Shut down system
    Key([mod, "shift", "control"], "x", lazy.spawn("systemctl poweroff"),
    ...
    Key([mod, "shift"], "s", lazy.spawn("rofimoji -f latin-1_supplement -r Special"), 
    ...
    # Function Keys
    Key([], "XF86AudioRaiseVolume", lazy.spawn(f"python {scripts}volume_up.py"),
        desc="Raise Volume"),
    Key([], "XF86AudioLowerVolume", lazy.spawn(f"python {scripts}volume_down.py"),
        desc="Lower Volume"),
    Key([], "XF86AudioMute", lazy.spawn(f"python {scripts}mute_toggle.py"),
        desc="Mute toggle"),
    Key([], "XF86MonBrightnessDown", lazy.spawn(f"python {scripts}brightness_down.py"),
        desc="Decrease Brightness"),
    Key([], "XF86MonBrightnessUp", lazy.spawn(f"python {scripts}brightness_up.py"),
    ...
    Key(["mod1"], "period", lazy.spawn("playerctl next"),
        desc="Switch to next track"),
    Key(["mod1"], "comma", lazy.spawn("playerctl previous"),
        desc="Switch to previous track"),
]
...
if xorg_flag:
    keys.extend(
        [
            # Toggle TouchPad
            Key(["mod1", "control"], "t", lazy.spawn(f"python {scripts}toggle-touchpad.py"),
                desc="Toggle touchpad on and off"),
            # Lock screen
            Key(["mod1", "control"], "l", lazy.spawn("betterlockscreen -l dimblur"),
                desc="Lock Screen"),
            # Switch between windows
            Key(["mod1"], "Tab", lazy.spawn("rofi -show window -theme catppuccin-macchiato"),
...
if wayland_flag:
    keys.extend(
        [
            #Take screenshot
            Key([], "Print", lazy.spawn(f"bash {scripts}/screenshot.sh"), desc="Screen grab"),
            # Lock screen
...
for i, workspace in enumerate(groups):
    keys.extend(
        [
            ...
            Key([mod], f"{i+1}", lazy.group[workspace.name].toscreen(),
            desc=f"Switch to group {workspace.name}"),

            # mod1 + shift + letter of group = switch to & move focused window to group
            Key([mod, "shift"], f"{i+1}", lazy.window.togroup(workspace.name, switch_group=True),
            ...
            Key([mod, "control", "shift"], f"{i+1}", lazy.window.togroup(workspace.name),
...
def InitWidgetsList():
    widgets_list = [
        ...
        widget.Clock(
            ...
            foreground = catppuccin['surface0'],
            format = "  %H:%M  %a %d.%m.%Y ",
            mouse_callbacks = {'Button1': lambda: qtile.cmd_spawn(f"xdg-open https://calendar.google.com")},
            **decorations_group
        ),
        widget.Spacer(
            background="ffffff00",
    ...
    if xorg_flag:
        ...
        widgets_list.append(
            widget.Wlan(
                ...
                interface="wlan0",      # get it with iwconfig
                update_interval=5,
                mouse_callbacks = {'Button1': lambda: qtile.cmd_spawn("networkmanager_dmenu")},
                **decorations_group
            )
    ...
    widgets_list.append(
        widget.TextBox(
            ...
            mouse_callbacks = {'Button1': lambda: qtile.cmd_spawn(f"bash {scripts}powermenu/type-1/powermenu.sh")},
            padding = 0,
            fontsize = 24,
            **decorations_group
        )
...
def InitScreens():
    ...
    if xorg_flag:
        screens_gen = [screenf(InitWidgetsScreen()), screenf(InitWidgetsScreenExternal())]
    elif wayland_flag:
...
wl_input_rules = {
    "1267:12866:Example Touchpad": InputConfig(natural_scroll=True, tap=True, dwt=True)
}
